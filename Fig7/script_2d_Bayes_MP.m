...
for i1 = 1:N1
    ...
    save(sprintf('%s_data.mat',mfilename),'N','K','MK','J','M2','disk_radius','gcenter_seed','gphi_seed','stim_DC','V0','V1','noise_values','lambda0','sigma0','c0','prefactor','exponent','threshold','FC_OPT','PFC_OPT','RUNTIMES_OPT');
...
plot([])
hold on
axis([noise_values(1) noise_values(end) 0 1])
xlabel('noise [mV]');
ylabel('performance');
title(sprintf('N = %d, K = %d, MK = %d, J = %d, M2 = %d',N,K,MK,J,M2))
p1 = plot(noise_values,FC_OPT,'color',light_blue,'linewidth',2);
p2 = plot(noise_values,PFC_OPT,'color',light_blue,'linewidth',2,'linestyle',':');
l0 = line([noise_values(1) noise_values(end)],[1/K 1/K],'color','r','linestyle',':');
