...
K_reference = 10; reference_index = find(K_values == K_reference);

N = 500; KMK = 100; J = 50; M1 = 20; M2 = 50;

maxiter = 10000; conv_crit = 1e-4; eta0 = 1e-8;

disk_radius = 90; gcenter_seed = 1; gphi_seed = 1;

stim_DC = 0.5; V0 = -60; V1 = 12; noise = 6; 
...
for i1 = 1:N1
    ...
    U1 = repmat(U_MEAN,M1,1)+noise*randn(KMK*J*M1,N);
    U2 = repmat(U_MEAN,M2,1)+noise*randn(KMK*J*M2,N);
...
save(sprintf('%s_data.mat',mfilename),'N','K_values','K_reference','reference_index','KMK','J','M1','M2','maxiter','conv_crit','eta0','disk_radius','gcenter_seed','gphi_seed','stim_DC','V0','V1','noise','lambda0','sigma0','c0','thresholds','FC','PFC','RUNTIMES','ITERATIONS','GBANK');
...
title({sprintf('N = %d, KMK = %d, J = %d, M1 = %d, M2 = %d',N,KMK,J,M1,M2),sprintf('convcrit = %g, eta0 = %g, resolution = %g [mV]',conv_crit,eta0,dth)})
N1 = length(K_values);
curve_plots = 1:N1;
legend_strings = {};
for i1 = 1:N1
    ...
    legend_strings{end+1} = sprintf('K = %d (%dmin)%s',K_values(i1),ceil(sum(RUNTIMES(i1,:))/60),ref);
