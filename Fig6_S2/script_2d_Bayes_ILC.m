...
maxdk = (length(probabilities)-1)/2;
PKK = zeros(3*K*MK,K*MK);
for k = 1:K*MK
    if k > 0.5*K*MK
       PKK(k-maxdk:k+maxdk,k) = probabilities;
    end
    PKK(K*MK+k-maxdk:K*MK+k+maxdk,k) = probabilities;
    if k < 2.5*K*MK
       PKK(2*K*MK+k-maxdk:2*K*MK+k+maxdk,k) = probabilities;
    end
end
PKK = PKK(K*MK+1:2*K*MK,:);
...
for i1 = 1:N1
    ...
    for ii = 1:I2
        ...
        PMKii_OPT = exp(SUMLOGii_kj-alpha0);
        PMK_OPT(:,ii) = sum(PMKii_OPT,2); % summa J
    end
    PMK_OPT = PMK_OPT./repmat(sum(PMK_OPT),K*MK,1); % normalization
    
    PMK2_OPT = PKK*PMK_OPT;
    
    PK_OPT = zeros(K,I2);
    for kk = 1:K
        for mk = 1:MK
            PK_OPT(kk,:) = PK_OPT(kk,:)+PMK2_OPT((kk-1)*MK+mk,:);
        end
    end
    ...
    save(sprintf('%s_data.mat',mfilename),'N','K','MK','J','M2','disk_radius','gcenter_seed','gphi_seed','stim_DC','V0','V1','thetasig','noise','lambda0','sigma0','c0','prefactor','exponent','thresholds','FC_OPT','PFC_OPT','RUNTIMES_OPT');
