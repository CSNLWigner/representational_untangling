...
% data: decoding performance from (optimal) firing rates (FR)
...
load('../Fig4/script_1d2d3d4d_data.mat')

OPTTH = [];
for i1 = 1:8
    [maxx,~] = maxfit(thresholds,FC(i1,:),3);
    OPTTH(end+1) = maxx;
end
...
gtheta = {'grid',0,180,N};
gphi = {'perm',0,360,gseed_values(i1)};
gsigma = {'const',sigma0};
GBANK = paramset(gx0,gy0,glambda,gtheta,gphi,gsigma);
GBANK(:,1:2) = randisk(disk_radius,N,gseed_values(i1));
...
for i2 = 1:N2
    ...
    FC_FR(i1,i2) = perfeval_fc(PK,Y2);
    PFC_FR(i1,i2) = perfeval_pfc(PK,Y2);
