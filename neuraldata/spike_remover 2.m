...
for n = 1:size(cells,2)
    ...
    for trial = 1:cells(n).trials
        ...
        varname_raw = sprintf('%s%d_trial%d_raw',cells(n).type,m,trial);
        eval([sprintf('%s = U_raw;',varname_raw)]);
        varname_peaks = sprintf('%s%d_trial%d_peaks',cells(n).type,m,trial);
        eval([sprintf('%s = I_peaks(2:end);',varname_peaks)]);
        varname_thresholds = sprintf('%s%d_trial%d_thresholds',cells(n).type,m,trial);
        eval([sprintf('%s = SP_thresholds;',varname_thresholds)]);
        varname_genpot = sprintf('%s%d_trial%d_genpot',cells(n).type,m,trial);
        eval([sprintf('%s = U_gen;',varname_genpot)]);
        varname_indices = sprintf('%s%d_trial%d_indices',cells(n).type,m,trial);
        save(mdatafile,varname_raw,varname_peaks,varname_thresholds,varname_genpot,varname_indices,'-append');
    end
end